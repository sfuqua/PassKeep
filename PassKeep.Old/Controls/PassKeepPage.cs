...
namespace PassKeep.Controls
{
    ...
    public abstract class PassKeepPage : Page
    {
        ...
        /// <param name="cts">A CancellationTokenSource used for cancelling the loading event.</param>
        /// <param name="indeterminate">Whether the loading progress is indeterminate.</param>
        protected void RaiseStartedLoading(string text, CancellationTokenSource cts, bool indeterminate = true)
        {
            if (StartedLoading != null)
            {
                StartedLoading(this, new LoadingStartedEventArgs(text, cts, indeterminate));
            }
        }
