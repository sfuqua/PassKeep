...
namespace PassKeep.KeePassLib
{
    ...
    public class VariantValue
    {
        ...
        static VariantValue()
        {
            ...
            {
                MemberInfo enumMember = typeof(Type).GetRuntimeField(type.ToString());
                TagAttribute tag = enumMember.GetCustomAttribute<TagAttribute>();

                Debug.Assert(tag != null);
                Debug.Assert(!TagMap.ContainsKey(tag.Value));
        ...
        public VariantValue(uint val)
        {
            ...
            this.data = ByteHelper.GetLittleEndianBytes(val);
        }
        ...
        public VariantValue(int val)
        {
            ...
            this.data = ByteHelper.GetLittleEndianBytes((uint)val);
        }
        ...
        public VariantValue(long val)
        {
            ...
            this.data = ByteHelper.GetLittleEndianBytes((ulong)val);
        }
        ...
        public VariantValue(byte[] val)
        {
            ...
            if (val.Length > 0)
            {
                System.Buffer.BlockCopy(val, 0, this.data, 0, val.Length);
            }
        ...
        public VariantValue(IBuffer data, byte tag)
        {
            ...
            {
                throw new ArgumentException("Unknown tag value " + tag, "tag");
            }
            ...
            switch (VariantType)
            {
                case Type.UInt32:
                    if (Size != 4)
                    {
                        throw new ArgumentException("Expected 4 bytes for " + VariantType + ", got " + Size, "data");
                    }
                    this.value = ByteHelper.BufferToLittleEndianUInt32(this.data, 0);

                    break;
                case Type.UInt64:
                    if (Size != 8)
                    {
                        throw new ArgumentException("Expected 8 bytes for " + VariantType + ", got " + Size, "data");
                    }
                    this.value = ByteHelper.BufferToLittleEndianUInt64(this.data, 0);

                    break;
                case Type.Bool:
                    if (Size != 1)
                    {
                        throw new ArgumentException("Expected 1 byte for " + VariantType + ", got " + Size, "data");
                    }
                ...
                case Type.Int32:
                    ...
                    this.value = (int)ByteHelper.BufferToLittleEndianUInt32(this.data, 0);
                ...
                case Type.Int64:
                    ...
                    this.value = (long)ByteHelper.BufferToLittleEndianUInt64(this.data, 0);

                    break;
                case Type.String:
                    this.value = Encoding.UTF8.GetString(this.data, 0, this.data.Length);
                    break;
                case Type.ByteArray:
                    this.value = this.data;
                    break;
        ...
        public byte GetTag()
        {
            MemberInfo enumMember = typeof(Type).GetRuntimeField(VariantType.ToString());
            TagAttribute tag = enumMember.GetCustomAttribute<TagAttribute>();
        ...
        public byte[] GetData()
        {
            ...
            if (Size > 0)
            {
                System.Buffer.BlockCopy(this.data, 0, value, 0, Size);
            }
        ...
        public void CopyTo(byte[] dest, int offset)
        {
            if (Size > 0)
            {
                System.Buffer.BlockCopy(this.data, 0, dest, offset, Size);
            }
        }
