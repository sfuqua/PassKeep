...
namespace PassKeep.KeePassLib
{
    ...
    public class HmacBlockHandler
    {
        ...
        public HmacBlockHandler(IBuffer key)
        {
            ...
            {
                throw new ArgumentException("The HMAC key must be 512 bits", "key");
            }
        ...
        public static IBuffer DeriveHmacKey(IBuffer compositeKey, IBuffer masterSeed)
        {
            ...
            {
                throw new ArgumentException("Composite key should be 32 bytes", "compositeKey");
            }

            if (masterSeed.Length != 32)
            {
                throw new ArgumentException("Master seed should be 32 bytes", "masterSeed");
            }
        ...
        public IBuffer GetMacForBlock(ulong i, IBuffer cipherText, uint offset, uint length)
        {
            ...
            Debug.Trace("Generating MAC value for block #" + i + ", data length " + length);
            if (length > 0)
            {
                Debug.Trace("data[0]: " + buffer[12] + ", data[n]: " + buffer[buffer.Length - 1]);
            }
            Debug.Trace("MAC[0]:  " + macValue.GetByte(0) + ", MAC[31]: " + macValue.GetByte(31));
            return macValue;
        }
        ...
        public async Task<IBuffer> ReadCipherBlockAsync(DataReader reader, ulong blockIndex)
        {
            ...
            Debug.Trace("Read HMAC block #" + blockIndex + "; block is " + loadedBytes + " bytes.");
            Debug.Trace("MAC[0]: " + actualHmacValue.GetByte(0) + ", MAC[31]: " + actualHmacValue.GetByte(31) + ", data[0]: " + cipherText.GetByte(0) + ", data[n]: " + cipherText.GetByte(loadedBytes - 1));
        ...
        public async Task WriteCipherBlockAsync(DataWriter writer, IBuffer cipherText, uint offset, uint length, ulong blockIndex)
        {
            ...
            {
                throw new ArgumentException("Block size is limited to int.MaxValue", "cipherText");
            }
            ...
            Debug.Trace("Wrote HMAC block #" + blockIndex + "; block is " + length + " bytes.");
            Debug.Trace("MAC[0]: " + hmacValue.GetByte(0) + ", MAC[31]: " + hmacValue.GetByte(31) + ", data[0]: " + cipherText.GetByte(offset) + ", data[n]: " + cipherText.GetByte(offset + length - 1));
        }
        ...
        public async Task WriteTerminatorAsync(DataWriter writer, ulong blockIndex)
        {
            ...
            Debug.Trace("Wrote HMAC terminator block #" + blockIndex + ".");
            Debug.Trace("MAC[0]: " + hmacValue.GetByte(0) + ", MAC[31]: " + hmacValue.GetByte(31));
        }
    }
}
