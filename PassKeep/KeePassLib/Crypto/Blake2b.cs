    ...
    public class Blake2b
    {
        ...
        public static ulong[][] GetDataBlocks(byte[] message, byte[] key)
        {
            ...
            if (key != null && key.Length > 0)
            {
                if (key.Length > 64)
                {
                    throw new ArgumentOutOfRangeException("key", "Key is limited to 64 bytes");
                }

                int keyBytes;
                keyBlock = GetSingleBlock(key, 0, out keyBytes);
                Debug.Assert(keyBytes == key.Length);
            }
            ...
            if (extraDataBlock)
            {
                ...
                Debug.Assert(bytesRead == leftoverDataBytes);
            }
        ...
        public static byte[] Hash(ulong[][] data, int inputBytes, byte keyBytes, byte hashBytes)
        {
            ...
            if (keyBytes > 64)
            {
                throw new ArgumentOutOfRangeException("keyBytes");
            }

            if (hashBytes == 0 || hashBytes > 64)
            {
                throw new ArgumentOutOfRangeException("hashBytes");
            }
        ...
        public static void Compress(ulong[] state, ulong[] block, ulong tHigh, ulong tLow, bool finalBlock)
        {
            ...
            if (state.Length != 8)
            {
                throw new ArgumentException("state must be 8 words long", "state");
            }
            ...
            if (block.Length != 16)
            {
                throw new ArgumentException("block must be 16 words long", "block");
            }
