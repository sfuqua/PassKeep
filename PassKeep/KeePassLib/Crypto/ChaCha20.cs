...
namespace PassKeep.KeePassLib.Crypto
{
    ...
    public class ChaCha20 : AbstractRng
    {
            ...
            : base(key)
        {
            ...
            {
                throw new ArgumentException("IV/Nonce must be 12 bytes", "initializationVector");
            }
        ...
        public static void QuarterRound(uint[] data, int ai = 0, int bi = 1, int ci = 2, int di = 3)
        {
            ...
            unchecked
            {
                a += b; d ^= a; d = ByteHelper.RotateLeft(d, 16);
                c += d; b ^= c; b = ByteHelper.RotateLeft(b, 12);
                a += b; d ^= a; d = ByteHelper.RotateLeft(d, 8);
                c += d; b ^= c; b = ByteHelper.RotateLeft(b, 7);
            }
        ...
        public static uint[] ConstructState(byte[] key, byte[] nonce, uint blockCount)
        {
            ...
            if (key.Length != 32)
            {
                throw new ArgumentException("Key must be 32 bytes long", "key");
            }

            if (nonce.Length != 12)
            {
                throw new ArgumentException("Nonce must be 12 bytes long", "nonce");
            }
        ...
        public static byte[] Block(uint[] state)
        {
            ...
            if (state.Length != 16)
            {
                throw new ArgumentException("State must be 16 words", "state");
            }
