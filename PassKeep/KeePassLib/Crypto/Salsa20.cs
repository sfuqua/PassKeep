...
namespace PassKeep.KeePassLib.Crypto
{
    ...
    public class Salsa20 : AbstractRng
    {
        ...
        // Per spec these values should use ASCII, but that doesn't exist in Encoding on Windows 8.
        // Thankfully it doesn't actually matter for these characters.
        private static readonly byte[] sigma = Encoding.UTF8.GetBytes("expand 32-byte k");
        private static readonly byte[] tau = Encoding.UTF8.GetBytes("expand 16-byte k");
            ...
            : base(seed)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (i < 2)
                {
                    uint subIv = ByteHelper.BufferToLittleEndianUInt32(keePassIv, i * 4);
                    Array.Copy(ByteHelper.GetLittleEndianBytes(subIv), 0, nonce, i * 4, 4);
                }

                uint subkeyL = ByteHelper.BufferToLittleEndianUInt32(key, i * 4);
                Array.Copy(ByteHelper.GetLittleEndianBytes(subkeyL), 0, lowerKey, i * 4, 4);

                uint subkeyU = ByteHelper.BufferToLittleEndianUInt32(key, (i * 4) + 16);
                Array.Copy(ByteHelper.GetLittleEndianBytes(subkeyU), 0, upperKey, i * 4, 4);
            }
        }
        ...
        public static uint[] QuarterRound(uint[] y, int offset = 0)
        {
            ...
            unchecked
            {
                z[1] = y[1 + offset] ^ ByteHelper.RotateLeft((y[0 + offset] + y[3 + offset]), 7);
                z[2] = y[2 + offset] ^ ByteHelper.RotateLeft((z[1] + y[0 + offset]), 9);
                z[3] = y[3 + offset] ^ ByteHelper.RotateLeft((z[2] + z[1]), 13);
                z[0] = y[0 + offset] ^ ByteHelper.RotateLeft((z[3] + z[2]), 18);
            }
