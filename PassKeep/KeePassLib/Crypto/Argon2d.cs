...
namespace PassKeep.KeePassLib.Crypto
{
    ...
    public class Argon2d
    {
            ...
            int parallelism, int tagLength, int memorySize, int iterations)
        {
            ...
            if (nonce.Length < 8)
            {
                throw new ArgumentOutOfRangeException("nonce", "nonce must be at least 8 bytes long");
            }
            ...
            {
                throw new ArgumentOutOfRangeException("secretValue", "The max key length is 32 bytes");
            }
        ...
        public byte[] GetHashParameters()
        {
            ...
            Debug.Assert(offset + this.associatedData.Length == parameterBuffer.Length);
        ...
        private void GetRefCoordinates(int currentLane, int currentBlock, int pass, int blocksPerSegment, int prevAddress, out int refLane, out int refBlock)
        {
            ...
            Debug.Assert(blocksPerSegment > 0);
            ...
            Debug.Assert(referenceBlocks > 0);
            ...
            Debug.Assert((int)relativePosition >= 0);
            Debug.Assert((int)relativePosition < referenceBlocks);
