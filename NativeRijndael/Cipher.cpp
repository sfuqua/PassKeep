// Slightly modified version of public domain Rijndael.cpp from KeePass's
...
int Cipher::Init(Mode mode, Direction dir, const uint8 *key, KeyLength keyLen, const uint8 *initVector)
{
...
int Cipher::BlockEncrypt(const uint8 *input, int inputLen, uint8 *outBuffer)
{
    int i, k, numBlocks;
...
int Cipher::PadEncrypt(const uint8 *input, int inputOctets, uint8 *outBuffer)
{
...
int Cipher::BlockDecrypt(const uint8 *input, int inputLen, uint8 *outBuffer)
{
    int i, k, numBlocks;
    uint8 block[16], iv[4][4];
...
int Cipher::PadDecrypt(const uint8 *input, int inputOctets, uint8 *outBuffer)
{
    int i, numBlocks, padLen;
    uint8 block[16];
    uint32 iv[4];
    ...
    if ((input == nullptr) || (inputOctets <= 0)) return 0;
...
void Cipher::KeySched(uint8 key[RD_MAX_KEY_COLUMNS][4])
{
    int j, rconpointer = 0;
...
void Cipher::Encrypt(const uint8 a[16], uint8 b[16])
{
    int r;
    uint8 temp[4][4];
...
void Cipher::Decrypt(const uint8 a[16], uint8 b[16])
{
    int r;
    uint8 temp[4][4];
